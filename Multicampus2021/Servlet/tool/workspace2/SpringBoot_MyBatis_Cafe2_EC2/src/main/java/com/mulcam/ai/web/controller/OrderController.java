...
public class OrderController {
	...
	@RequestMapping(
			value = "order2.jes", 
			method = { RequestMethod.POST }, 
			consumes = "application/json;charset=utf-8") // 이렇게 받아오겠다 

	@ResponseBody
	public String order2(@RequestBody List<Map<String, Object>> param) { 
		// 파라미터는 제이슨배열형태로 도착. [{"name":"핫 아메리카노", "quantity":"1"}, {"name":"핫 카페라떼", "quantity":"2"}]
		// 이런 형태를 받아오려면 리스트에 맵을 줘야 함 
		
		JSONObject json = null;

		ArrayList<OrderVO> list = new ArrayList<OrderVO>();
		for (Map<String, Object> map : param) { // param의 각 Map을 돌아서 map이라는 이름으로 받아옴
			String prodname = (String) map.get("name"); //
			// System.out.println(rawName);

			try {
				prodname = URLDecoder.decode(prodname, "UTF-8"); // 한글처리된 prodname
